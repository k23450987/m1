...
public class DeptController {
    ...
    public String find(Model model) {
        // 查询到所有部门详细信息
        List<Dept> lists = deptService.select();
        // 传值到页面
        model.addAttribute("depts", lists);
        return "/manage/deptManage/list";
    ...
    public String toInsert(Model model) {
        ...
        List<Dept> depts = deptService.selectFather();
        // 传值到页面
        model.addAttribute("depts", depts);
        return "/manage/deptManage/add";
    }
    ...
    public String insert(Dept dept, Model model, RedirectAttributes redirectAttributes) {
        // System.out.println(dept_name);
        // 判断部门名称是否存在
        if (deptService.selectByName(dept.getName()).size() != 0) {
            // 名称已存在,添加错误信息
        ...
        //List<Dept> newDepts = deptService.selectByName(dept.getDept_name());
        ...
        //		deptService.updateParent(dept2);
        //	}
        //}
    ...
    public String verifyDept(String dept_name) {
        if (StringUtils.isNotBlank(dept_name)) {
            System.out.println(dept_name);
            List<Dept> depts = deptService.selectByName(dept_name);
            if (depts.size() != 0) {
                return "fail";
            }
            return "success";
    ...
    public String toUpdate(Integer dept_id, Model model) {
        ...
        // 传到页面的下拉菜单
        model.addAttribute("depts", depts);
        return "/manage/deptManage/update";
    }
    ...
    public String update(Dept dept, RedirectAttributes redirectAttributes) {
        ...
        // List<Dept> newDepts = deptService.selectByName(dept.getName());
        /*for (Dept dept2 : newDepts) {
			// 判断添加时该部门是否为顶级部门,通过查询fdept_id是否为0
			if (dept2.getFdept_id() == 0) {
				// System.out.println(dept2);
