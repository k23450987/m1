...
<div id="searchid" align="center" style="padding: 5px">
		...
		<span> 
			<label for="selectId"></label><label for="selectId"></label><select name="params" id="selectId">
...
<script type="text/javascript">
    ...
    $(function () {
        $.getJSON("<%=path%>/findData2?tableId=" + tableId + "&pageNum=1", function (json) {
            // 计算总页数
            var page = Math.ceil(json.datas.total / 10);
            // 自由库列信息
            var columns = json.cols;
            // 自由库数据
            var datas = json.datas.list;
            // 判断自由库中有数据吗
            if (datas && datas.length > 0) {
                ...
                for (var i = 0; i < columns.length; i++) {
                    ...
                    // 添加到 thead 中
                    $("#table").find("thead tr").append("<td>" + column + "</td>");
                }
                //alert(page);
    ...
    function search_like(num) {
        var $text = $("#searchid").find("span input").val();
        var $colid = $("#selectId").find("option:selected").val();
        var $pagesize = $("#pagesizeid").find("option:selected").val();
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (datas && datas.length > 0) {
                    var pages = Math.ceil(result.datas.total / $pagesize);
                    ...
                    $('#pagination-demo').twbsPagination({
                        ...
                        onPageClick: function (event, page) {
                            $.getJSON("selectLike.action", {
                                ...
                                var tbody;
                                for (var i = 0; i < json.datas.list.length; i++) {
                                    tbody += "<tr>";
                                    for (var j = 0; j < json.cols.length; j++) {
                                        var data = json.datas.list[i][json.cols[j].params];
                                        tbody += "<td width=100px>" + (data ? data : " ") + "</td>"
                                    }
                                    tbody += "</tr>";
                                }
